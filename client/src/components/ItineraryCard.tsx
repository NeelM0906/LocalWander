...
const ItineraryCard = ({ itinerary, onSelect }: ItineraryCardProps) => {
  ...
  return (
    <div 
      className="bg-card rounded-lg border border-secondary hover:border-accent transition-all duration-300 cursor-pointer group flex flex-col overflow-hidden"
    ...
    >
      <div className="w-full h-48 bg-secondary flex items-center justify-center overflow-hidden">
        {imageUrl ? (
          <img src={imageUrl} alt={itinerary.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
        ) : (
          <div className="w-full h-full flex items-center justify-center bg-secondary">
            <LogoIcon className="w-16 h-16 text-muted-foreground" />
          </div>
        )}
      </div>
      <div className="p-6 flex flex-col flex-grow">
        ...
        <p className="text-muted-foreground mb-4 text-sm flex-grow">{itinerary.description}</p>
        
        <div className="flex items-center justify-between text-sm text-muted-foreground border-t border-secondary pt-4 mt-auto">
